...
| 목록 카드의 상태 라벨 | 색 그대로. **카드당 색이 나오는 곳은 여기 한 곳** |
| 만료 실선 | 색 그대로 + `opacity .5`. 너비가 남은 시간 |
| 방 구성 띠 | 인원 비율만큼 색이 차지 |

**상태색을 브랜드색으로 쓰지 않는다.** 버튼과 링크는 무채색 또는 흰색이다.
색이 의미를 잃으면 상태 표시가 안 읽힌다.

**한 카드에 색은 한 계열만.** 목록 카드는 자기 상태색 하나만 쓴다.
색을 빼면 화면이 심심해지고, 아무 데나 쓰면 상태가 안 읽힌다. 규칙은 "색은 상태를 뜻할 때만"이지 "색을 쓰지 말 것"이 아니다.

### 색이 아닌 것으로 가르는 것

| 신호 | 방법 |
|---|---|
| 물리적으로 가까움(`near`) | 카드 배경을 `--surface` → `--surface-hi` 한 단계 밝게 |
| 남은 시간 | 카드 전체 `opacity` + 하단 실선 너비 |
| 무리 짓기 | `닿는 거리` / `조금 떨어진 곳` 라벨로 목록을 쪼갠다 |

근접도에까지 색을 주면 상태색과 싸운다. 밝기로 가른다.
