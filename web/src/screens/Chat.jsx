...
export default function Chat() {
  ...
  // message to somebody new needs discovery on and a valid observation.
  ...
  useLayoutEffect(() => {
    ...
    } else if (nearBottom.current || previous.current === null || messages.at(-1)?.sender_id === me.user_id) node.scrollTop = node.scrollHeight;
  ...
  return <section className="screen screen-chat">
    ...
    <div className="bubbles" ref={list} role="log" aria-label="대화 내용" aria-live="polite" aria-relevant="additions text"
      ...
      {messages.map((message) => <div key={message.id} className={'message-row ' + (message.sender_id === me.user_id ? 'mine' : '')}>
        <p className={'b ' + (message.sender_id === me.user_id ? 'me' : 'you')}><span className="sr-only">{message.sender_id === me.user_id ? '나' : peer?.nickname}: </span>{message.text}</p>
