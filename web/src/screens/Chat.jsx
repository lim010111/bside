...
export default function Chat() {
  ...
  return <section className="screen screen-chat">
    <PageHeader title={peer?.profile.nickname ?? '대화'} onBack={actions.goBack} action={peer && <button className="text-button" onClick={() => actions.navigate('detail', peer.user_id)}>소개</button>} />
    ...
    <form className="composer" onSubmit={submit}>
      <div className="grow"><label className="sr-only" htmlFor="message">메시지</label><textarea ref={composer} id="message" className="field" rows={1} placeholder={blocked ? '주변 발견을 켜면 보낼 수 있어요' : '메시지를 입력하세요'} value={draft}
        disabled={sending || !peer || blocked} aria-describedby={count >= 1800 ? 'message-count' : undefined} aria-invalid={count > LIMITS.text}
        onChange={(event) => actions.setDraft(targetId, event.target.value)} />
        {count >= 1800 && <span id="message-count" className={'counter' + (count > LIMITS.text ? ' danger' : '')}>{count.toLocaleString()} / 2,000</span>}
      </div>
