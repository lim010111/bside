...
**추천은 누가 근처에 있는지를 바꾸지 않는다.** 평가가 아직 없거나 실패해도 관측된 사용자는 그대로 목록에 남는다. 평가는 응답을 기다리게 하지 않는다. 서버는 캐시에 있는 것만 담아 즉시 응답하고 나머지는 백그라운드에서 평가하므로, 처음 보는 사람은 `pending`으로 왔다가 다음 관측에서 채워진다. 클라이언트는 이 갱신을 목록을 다시 그리는 신호로 다루고, `pending`을 오류나 낮은 점수로 표시하지 않는다.
...
API v0.1·프론트엔드·Android BLE 구현과 그 검증은 각 구현 문서를 따른다. 추천 알림(푸시)과 백그라운드 동작 검증, SSE/WebSocket 실시간 전달은 후속 범위다. 이벤트 룸·QR·운영자 종료·관리자 기능은 현재 계약에 포함하지 않는다.
