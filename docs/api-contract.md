...
`POST /me/push-token`으로 기기의 FCM 토큰을 등록한다. **토큰은 기기가 아니라 사용자 단위로 보관한다.** 같은 기기가 다른 사용자로 다시 등록하면 토큰은 새 사용자에게 넘어가고 이전 사용자에게서는 사라진다(`app/scripts/put_push_token.lua`). 재설치나 기기 양도 뒤에 이전 사용자의 메시지가 계속 그 기기로 가는 것을 막는 유일한 장치다. FCM이 토큰을 죽었다고 답하면 서버가 그 자리에서 지운다.
...
푸시는 켜고 끄는 사용자 설정이 아직 없다. 지금은 앱을 지우면 FCM이 토큰을 죽었다고 답하고 서버가 지우는 것이 유일한 해지 경로다.
