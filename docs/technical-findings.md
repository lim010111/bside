...
상태: **공식 문서 조사 완료, 실기기 검증 미실시.** 스택 선택은 위임받았으며, BLE 휴대폰 간 발견을 유지하는 안에는 Kotlin + Jetpack Compose를 추천한다. BLE는 **채택 확정**이다(Capacitor + peripheral 플러그인). 근거는 정밀도가 아니라 프라이버시 — GPS와 달리 서버에 좌표가 올라가지 않는다. [protocol.md](../spec/protocol.md) 0번 참조. 사용자 지시에 따라 현재 작업 환경은 플랫폼 선택의 근거로 사용하지 않는다. 테스트 기기는 선택한 구성을 검증하기 위해 별도로 확인한다. 아래 내용은 구현 완료 보고가 아니다.
...
제품의 BLE 필수 여부와 기기를 확인한 뒤 **45~60분으로 제한한 검증**을 제안한다. 아래 수치는 팀의 합의를 받지 않은 후보 기준이다.
...
세션 식별 방식, 광고에 넣을 정보와 연결 후 읽을 정보의 구분, 공개 대상, 프로필·상태의 수명, 메시지 크기와 중복 제거, 수신 확인, 재연결·만료 처리가 미정이다. 제품 흐름을 정하기 전에 프로토콜을 확정하지 않는다.
